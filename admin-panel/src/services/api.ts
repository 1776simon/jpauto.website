...
class ApiService {
   ...
   * Calls backend endpoint which handles:
   * 1. Fetching the main photo
   * 2. Processing image with banner (title status, price, company info)
   * 3. Uploading bannered image to R2
   * 4. Inserting at position 0, keeping original as backup
   */
  async applyBannerToMainPhoto(vehicleId: number | string): Promise<{ images: string[] }> {
    return this.request<{ images: string[] }>(`/api/inventory/${vehicleId}/apply-banner`, {
      method: 'POST',
    });
  }
